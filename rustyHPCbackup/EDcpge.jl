...
function EDcpge(ϵ, ω1, ω2, Es, hxp, hyp, hzp; σ=0.01, η=0.01)
	...
	resmat = zeros(ComplexF64,N,N) #spzeros(ComplexF64,N,N)
	#Mdelta, G11, G12, G21 = Matrix(Mdelta),Matrix(G11), Matrix(G12), Matrix(G21)
	#G22, G31, G32 = Matrix(G22), Matrix(G31), Matrix(G32)
	#hxp, hyp, hzp = Matrix(hxp), Matrix(hyp), Matrix(hzp)
