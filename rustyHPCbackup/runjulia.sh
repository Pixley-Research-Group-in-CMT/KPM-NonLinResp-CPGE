...
#SBATCH --job-name=tTest           # Assign a short name for the job
#SBATCH --nodes=1                      # Number of nodes you require
#SBATCH --ntasks-per-node=1            # Total number of tasks across all nodes
#SBATCH --cpus-per-task=64             # cpu-cores per task (>1 if multithread tasks)
###SBATCH --constraint=icelake         # Constraint the type of node broadwell/skylake/rome
...
srun julia $HOME/CPGE/KPMGammamnpMymodel.jl $SLURM_ARRAY_TASK_ID @> output${SLURM_ARRAY_TASK_ID}.log
