...
#SBATCH --partition=gpu          # Partition (job queue) main gpu

#SBATCH --requeue                 # Return job to the queue if preempted

#SBATCH --job-name=CPL18       # Assign a short name to your job
...
#SBATCH --constraint="volta|titan|pascal" # add constraint to the GPU node; volta|pascal seems to have too small memory

#SBATCH --mem=64000                # Real memory (RAM) required (MB)

#SBATCH --time=24:00:00           # Total run time limit (HH:MM:SS)

#SBATCH --array=1-25:1           # Use job array ID as a variable
