...
function BuildHdis(L;W=0.0,twists=[0.0,0.0,0.0], D=6.0)
	...
	Ham = TwoBandModel(Ls, t, M, γ;boundary=boundary)
	Jx, Jy, Jz = velocityOperator(Ls,Ham)
	H_norm = Ham./D
	return H_norm,Jx,Jy,Jz
end

function obtainGamma(H_norm,Jx,Jy,Jz;NC=2^8,D=6.0,NR=1)
	NH = H_norm.n
	mu_3d_xyz = zeros(ComplexF64, NC, NC, NC)
	psi_in_l = exp.(2pi * 1im * rand(H_norm.n, NR));
	KPM.normalize_by_col(psi_in_l, NR)
	psi_in_r = psi_in_l
	KPM.kpm_3d!(H_norm, Jx, Jy, Jz, NC, NR, NH, mu_3d_xyz, psi_in_l, psi_in_r; verbose=1, arr_size=128)
	return mu_3d_xyz
end

L = 100
@time H_norm, Jx,Jy,Jz = BuildHdis(L);
runtime = @elapsed obtainGamma(H_norm,Jx,Jy,Jz;NC=2^7);

runsam = parse(Int64,ARGS[1])

pt = "/scratch/aw666/CPGEdata/"  # amarel
fn = pt*"Gamma$(L)NC27GPUrunsam$(runsam)"*".jld2"
