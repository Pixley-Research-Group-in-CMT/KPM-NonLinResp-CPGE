...
t = 1.0
gamma = 0.0  # 0.8
twists = [0,0,0] #[pi,pi,pi]
boundary = exp.(1im.*twists)
@time H0 = TwoBandModel(Ls, t, M, gamma;boundary=boundary)
#@time H0old = TwoBandModelSlow(Ls, t, M, gamma;boundary=boundary)
@time Jx, Jy, Jz = velocityOperator(Ls,H0)
disorder = generateDisorder(Ls, 0.5)
@time Jxn, Jyn, Jzn = velocityOperator(Ls,H0+spdiagm(disorder))
#=
start = now()
OBC=false
tbc_theta = 0.5*ones(3) #zeros(3) #rand(3) # twist angles in [0,1] \pi implicit
...
tail = now()
@show tail-start
#save("spH0L100M2gam0.jld2","H0",Hsp,"Ls",Ls,"M",M,"gamma",gamma)
=#
